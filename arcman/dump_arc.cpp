...
/*
 * gup_result dump_archive::create_archive(const char *name, OPTIONS *options,
...
gup_result dump_archive::write_main_header(const mainheader *header)
{
	...
	case MHDR_ARJ:						/* Header is already an ARJ main header. */
		main_hdr = (const dump_mainheader *) header;
		break;
	default:							/* Other main header, create an ARJ main header. */
		main_hdr = (const dump_mainheader *) init_main_header(header->get_comment());
		new_allocated = TRUE;
		break;
	}
	...
	if (cur_main_hdr == NULL)
		cur_main_hdr = new dump_mainheader(*main_hdr);
	...
	if (new_allocated)
		delete (dump_mainheader *) main_hdr;
...
gup_result dump_archive::write_file_header(const fileheader *header)
{
	...
	case FHDR_ARJ:						/* Header is already an ARJ file header. */
		file_hdr = (const dump_fileheader *) header;
		break;
	default:							/* Other file header, create an ARJ main header. */
		file_hdr = (const dump_fileheader *) init_file_header(header->get_filename(),
	...
	if (new_allocated)
		delete (dump_fileheader *) file_hdr;
...
mainheader *dump_archive::read_main_header(gup_result &result)
{
							   ...
							   (arj_header_size <= HEADERSIZE_MAX)))
	{
		...
		/*
		 * No error or CRC error. Create a dump_mainheader object using
		 * the data read from file.
		 */

		main_hdr = new dump_mainheader(NULL);
...
fileheader *dump_archive::read_file_header(gup_result &result)
{
		...
		(arj_header_size <= HEADERSIZE_MAX)))
	{
		...
		/*
		 * No error or CRC error. Create a dump_fileheader object using
		 * the data read from file.
		 */

		file_hdr = new dump_fileheader(NULL, NULL);
 ...
 *
 * Allocate an dump_mainheader structure and initiate it.
...
mainheader *dump_archive::init_main_header(const char *comment)
{
	...
	main_hdr = new dump_mainheader(comment);
	main_hdr->set_filename(cur_volume->name());

	return (mainheader *) main_hdr;
}

fileheader *dump_archive::init_file_header(const char *filename,
										  const char *comment,
										  const osstat *stat)
{
	dump_fileheader *file_hdr;
	...
	if (stat == NULL)
		file_hdr = new dump_fileheader(filename, comment);
	else
		file_hdr = new dump_fileheader(filename, comment, stat);
...
/*
 * uint32 dump_archive::post_proc_crc(uint32 crc)
 *
...
gup_result dump_archive::encode(fileheader *header, int infile)
{
	...
	if ((result = archive::encode(header, infile)) == GUP_OK)
	{
		...
		if ((st.pack_str.mv_mode) && (st.pack_str.mv_next))
			((dump_fileheader *) header)->flags |= VOLUME_FLAG;
	}
