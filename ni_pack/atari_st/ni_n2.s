...
decompr_loop:
     ...
     move.b  (A0)+,D0         ; refill bit buffer
...
decompr_select:
     ...
     move.b  (A0)+,D1         ; insert bits, dus is D1 max 2^23
...
_e_1:
     ...
     move.b  (A0)+,D0         ; refill bitbuffer
     addx.b  D0,D0            ; get bit
_e_2:
     addx.w  D2,D2            ; schuif bit in D2, D2=-1..-4
     lea     0(A1,D3.l),A2    ; src ptr in A2
     addq.w  #2,D2            ;
...
decompr_large_mlen:
     move.b  (A2)+,(A1)+      ; in ieder geval 2 bytes + D2 bytes
     move.b  (A2)+,(A1)+      ;
     cmp.l   A3,D3            ; lange offset?
     bcs.s   L_copy2          ; ja, twee extra bytes
     move.b  (A2)+,(A1)+      ; nee, 1 extra byte
