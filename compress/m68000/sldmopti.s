...
insert2nmm_q_end:
	...
	movel   d2,a4@            | left en right = NNODE
	moveml  sp@+,d2-d5/a2-a6  | restore registers
	rts                         | done
|void insertnmm(word nkey, word nnode, word hash, struct packstruct *com)
_insert2nmm:
insert2nmm:
	moveml  d2-d5/a2-a6,sp@-  | save registers
	movel   sp@(40),d0          | nkey
	movel   sp@(44),d1          | nnode
	movel   sp@(48),d2          | hash
	movel   sp@(52),a0          | *com
	...
	moveq    #0,d2              | clear d2
	movew   a5@,d2            | current
	beq    insert2nmm_q_end   | current = 0 stoppe!
	movew   d1,a5@            | root[hash]=nnode
...
ll_0005:
	...
	movew   a4@,d2            | current=tp->c_right
	bne      ll_0000    | next
...
insertnmm:
	...
	moveq    #0,d2              | clear d2
	movew   a5@,d2            | current
	beq    insertnmm_q_end    | current = 0 stoppe!
	movew   d1,a5@            | root[hash]=nnode
	moveq    #0,d5              | clear voor tp->key
ll_0007:
	movel   d2,d4              | current
...
ll_0013:
	...
	movew   d2,a2@(6,d0:l)      | tree[left].c_right=current
	movel   d2,d0              | right=current
	movew   a4@(2),d2           | current=tp->right
	bne      ll_0007    | next
