/*
  ...
  Optimized c-functions for Atari ST
...
void free_crc32_table(unsigned long* table)
{
  if(table!=NULL)
  {
    free(table);
...
void *make_crc32_table(uint32* crc_poly_p)
{
  uint32 *crc_table;
  /*
...
void *make_crc16_table(uint16* crc_poly_p)
{
  uint16 *crc_table;
  /*
