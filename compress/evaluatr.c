...
#ifndef NOT_USE_STD_backmatch_medium
    ...
    if(len!=0)
    {
      ...
      if(*pp==*qq)
      {
        ...
        if(hist>=0)
        {
          hist_struct *histp=com->hist[hist];
          uint8 ptrlen=LOG(com->ptrp[-2]);
          uint16 origlen=(uint16)(com->charp[-2]-NLIT+MIN_MATCH-2); /* correctie voor +2 in encode_big */
          while(LOG(histp->ptr)<ptrlen)
          {
            if((origlen-histp->match) <=len)
            { /* back macth! */
              ...
              if(com->charp[-3]>=NLIT)
              {
                com->bmp--;
                com->charp--;
                backmatch_medium((uint16)(pos+ptr-origlen-1), (uint16)com->ptrp[-2], -1, com);
                com->charp++;
                ARJ_Assert((com->charp[-3]-NLIT+MIN_MATCH-2)<=com->max_match);
                ARJ_Assert((com->charp[-3]-NLIT+MIN_MATCH)>=MIN_MATCH);
              }
              ARJ_Assert((com->charp[-2]-NLIT+MIN_MATCH-2)<=com->max_match);
              ARJ_Assert((com->charp[-2]-NLIT+MIN_MATCH)>=MIN_MATCH);
              ARJ_Assert((com->charp[-1]-NLIT+MIN_MATCH-2)<=com->max_match);
...
#ifndef NOT_USE_STD_backmatch_small

int backmatch_small(uint16 pos, uint16 ptr, int hist, packstruct* com)
{
  if(com->charp[-2]>=NLIT)
  { /* only backmatch if previous char was a match */
    uint8 *pp=com->dictionary+ --pos;
    uint8 *qq;
    int max_len=com->max_match-com->charp[-1]+NLIT-MIN_MATCH;
    if(max_len==0)
    {
      *com->bmp++=0;
      return 0;
    ...
    qq=com->dictionary+pos-1;
    {
      if(*pp==*qq)
      {
        ...
        {
          uint8* origp=qq;
          uint8 orig=qq[max_len];
          qq[max_len]=~pp[max_len]; /* sentinel */
          do
          {
            if(*--pp!=*--qq) break;
            if(*--pp!=*--qq) break;
...
#ifndef NOT_USE_STD_encode_small

#define SMALL_DIC_DELTA_SIZE 1024
#undef BACKMATCH_ENCODESIZE
#define BACKMATCH_ENCODESIZE backmatch_small

gup_result encode_small(packstruct *com)
{
  uint16 huffbuf = com->hufbufsize + 2;  /* +2 voor pointerSWAP, pswp kan 2 waarde terughalen */
  uint16 current_pos = 0;                /* wijst de te packen uint8 aan        */
  uint16 refill_pos = SMALL_DIC_DELTA_SIZE;
  unsigned long bytes_to_do;
  signed long refill_count = SMALL_DIC_DELTA_SIZE;
  ...
  { /*- dictionary buffer vullen */
    ...
    if ((byte_count = com->arj_read(com->source_file, SMALL_DIC_DELTA_SIZE+1024, com->dictionary)) < 0)
    {
      return GUP_READ_ERROR; /* ("Read error"); */
    }
    else
    {
      com->crc=com->crc_func(com->dictionary, byte_count, com->crc, com->crc_table);
      #ifndef PP_AFTER
  ...
  *com->charp++ = com->dictionary[current_pos++];
  ...
  if(com->speed==2)
  {
    ...
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      match0 = find_dictionary_small_fast(current_pos, com);
      if(match0>0)
      {
        ...
        else
        {
          if (match0 > 0)
          {
            c_codetype match1;
            ptr0 = com->best_match_pos;
            match1 = find_dictionary_small_fast(MS(current_pos + 1), com);
      ...
      {
        long byte_count;
        refill_count += SMALL_DIC_DELTA_SIZE;
        if ((byte_count =
             com->arj_read(com->source_file, SMALL_DIC_DELTA_SIZE, com->dictionary + refill_pos+1024)) < 0)
        {
          return GUP_READ_ERROR; /* ("Read error"); */
        }
        else
        {
          if (byte_count)            /* byte count kan ook nul zijn */
          {
            com->crc=com->crc_func(com->dictionary + refill_pos+1024, byte_count, com->crc, com->crc_table);
            if((refill_pos&0x3fff)==0)
            {
              #ifndef PP_AFTER
              com->print_progres(0x4000);
              #endif
            }
          }
          bytes_to_do+=byte_count;
        }
        refill_pos = MS(refill_pos + SMALL_DIC_DELTA_SIZE);
        if (refill_pos == 0)
        {
          memcpy(com->dictionary-4,com->dictionary + com->tree_size-4, 1024+4);
        }
      }
    }
  }
  else if(com->speed==1)
  {
    ...
    while (bytes_to_do)
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      match0 = find_dictionary_small(current_pos, com);
      if(match0>0)
      {
        if ((unsigned long)match0 >= bytes_to_do)
        {
        ...
        else
        {
          if (match0 > 0)
          {
            c_codetype match1;
            ptr0 = com->best_match_pos;
            match1 = find_dictionary_small(MS(current_pos + 1), com);
            if ((unsigned long)match1 >= bytes_to_do)
            {
              match1 = (c_codetype)(bytes_to_do - 1);
              if (match1 < MIN_MATCH)
  ...
  { /*- mode = 1, 2 of 7 */
    ...
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      for(;;)
      {
        ...
        if ((unsigned long)match0 >= bytes_to_do)
        {
          int diff = (int)(match0 - bytes_to_do);
        ...
        if ((unsigned long)match1 >= bytes_to_do)
        {
          int diff = (int)(match1 - bytes_to_do + 1);

          match1 = (uint16) bytes_to_do - 1;
          if (ptrswap > 0)
          {
            swapmatch1 -= diff;
            if (swapmatch1 < MIN_MATCH)
        ...
        else
        {
          ...
          if (match1 == match0)
          {
            match2 = find_dictionary_small(MS(current_pos + match0), com);
            if (match2 == 0)
            {
              /*
            ...
            else
            {
              ...
              match3 = find_dictionary_small(MS(current_pos + match0 + 1), com);
              hist3 = com->hist_index;
              ptr3 = com->best_match_pos;
              match4 = match3;
              hist4 = com->hist_index;
              ptr4 = ptr3;
              match5 = find_dictionary_small(MS(current_pos + match1 + 2), com);
          ...
          else
          {
            ...
            hist3 = com->hist_index;
            ptr3 = com->best_match_pos;
            match4 = find_dictionary_small(MS(current_pos + match1 + 1), com);
            hist4 = com->hist_index;
...
#ifndef NOT_USE_STD_encode_medium

#define MEDIUM_DIC_START_SIZE 32768UL
#define MEDIUM_DIC_DELTA_SIZE 1024
#undef BACKMATCH_ENCODESIZE
#define BACKMATCH_ENCODESIZE backmatch_medium

gup_result encode_medium(packstruct *com)
{
  uint16 huffbuf = com->hufbufsize + 2;  /* +2 voor pointerSWAP, pswp kan 2 waarde terughalen */
  uint16 current_pos = 0;                /* wijst de te packen byte aan        */
  uint16 refill_pos = MEDIUM_DIC_START_SIZE;
  unsigned long bytes_to_do;
  signed long refill_count = MEDIUM_DIC_START_SIZE;
  ...
  { /*- dictionary buffer vullen */
    ...
    if ((byte_count = com->buf_read_crc(MEDIUM_DIC_START_SIZE+1024, com->dictionary, com->brc_propagator)) < 0)
    {
      return GUP_READ_ERROR; /* ("Read error"); */
    }
    else
  ...
  *com->charp++ = com->dictionary[current_pos++];
  ...
  if(com->speed==2)
  {
    ...
    while (bytes_to_do)
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      match0 = find_dictionary_medium_fast((uint16)(current_pos+2), com);
      if(match0>0)
      {
        if ((unsigned long)match0 >= bytes_to_do)
        {
        ...
        else
        {
          if (match0 > 0)
          {
            c_codetype match1;
            ptr0 = com->best_match_pos;
            match1 = find_dictionary_medium_fast((uint16)(current_pos + 3), com);
            if ((unsigned long)match1 >= bytes_to_do)
            {
              match1 = (c_codetype)(bytes_to_do - 1);
              if (match1 < MIN_MATCH)
      ...
      {
        long byte_count;
        refill_count += MEDIUM_DIC_DELTA_SIZE;
        if ((byte_count =
             com->buf_read_crc(MEDIUM_DIC_DELTA_SIZE, com->dictionary + refill_pos+1024, com->brc_propagator)) < 0)
        {
          com->max_match+=2; /* herstel max_match */
          return GUP_READ_ERROR; /* ("Read error"); */
        }
        else
        {
          if (byte_count!=0)            /* byte count kan ook nul zijn */
          {
            ...
            {
              #ifndef PP_AFTER
              com->print_progres(0x4000, com->pp_propagator);
              #endif
            }
          }
          bytes_to_do+=byte_count;
        }
        refill_pos += MEDIUM_DIC_DELTA_SIZE;
        if (refill_pos == 0)
        {
          memcpy(com->dictionary-4,com->dictionary + DIC_SIZE_MEDIUM-4, 1024+4);
        }
      }
    }
  }
  else if(com->speed==1)
  {
    ...
    while (bytes_to_do)
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      match0 = find_dictionary_medium((uint16)(current_pos+2), com);
      if(match0>0)
      {
        if ((unsigned long)match0 >= bytes_to_do)
        {
        ...
        else
        {
          if (match0 > 0)
          {
            c_codetype match1;
            ptr0 = com->best_match_pos;
            match1 = find_dictionary_medium((uint16)(current_pos + 3), com);
            if ((unsigned long)match1 >= bytes_to_do)
            {
              match1 = (c_codetype)(bytes_to_do - 1);
              if (match1 < MIN_MATCH)
    ...
    { /*- hoofd_lus, deze lus zorgt voor al het pack werk */
      for(;;)
      {
        ...
        else
        {
          ...
          if (match1 == match0)
          {
            match2 = find_dictionary_medium((uint16)(current_pos + match0 + 2), com);
            if (match2 == 0)
            {
              /*
               * literal pointer length, als het andersom moet dan wordt
            ...
            else
            {
              hist2 = com->hist_index;
              ptr2 = com->best_match_pos;
              match3 = find_dictionary_medium((uint16)(current_pos + match0 + 3), com);
              hist3 = com->hist_index;
              ptr3 = com->best_match_pos;
              match4 = match3;
              hist4 = com->hist_index;
              ptr4 = ptr3;
              match5 = find_dictionary_medium((uint16)(current_pos + match1 + 4), com);
              hist5 = com->hist_index;
              ptr5 = com->best_match_pos;
              if (match3 > match2)
              {
          ...
          else
          {
            ...
            hist3 = com->hist_index;
            ptr3 = com->best_match_pos;
            match4 = find_dictionary_medium((uint16)(current_pos + match1 + 3), com);
            hist4 = com->hist_index;
            ptr4 = com->best_match_pos;
            match5 = find_dictionary_medium((uint16)(current_pos + match1 + 4), com);
            hist5 = com->hist_index;
