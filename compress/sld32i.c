	...
	{ /* check for matches at pointer history positions */
		...
		for(i=0; i<MAX_PTR_HIST; i++)
		{
			...
			if((match_pos<pos) && (match_pos>DICTIONARY_START_OFFSET))
			{
				...
				while(com->dictionary[p+match]==com->dictionary[q+match])
				{
					match++;
				}
				if(match>BEST_MATCH(best_match))
			...
			*parent=pos;
			...
			{ /* jaag de link pointers na op zoek naar pointer reuse */
				...
				while((match_pos!=NO_NODE) && (repeat_pos!=NO_NODE))
				{
					...
					{
						index_t p=repeat_match;
						index_t q=repeat_pos;
						match_t i=repeat_len;
						do
						{
							i--;
							if(com->dictionary[p+i]!=com->dictionary[q+i])
							{
								break;
							}
						} while(i>0);
