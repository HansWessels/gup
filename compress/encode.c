...
gup_result init_encode(packstruct *com)
{
  ...
  switch(com->mode)
  {
    ...
    case NI_MODE_1:
      ...
      com->close_packed_stream=close_n1_stream; /* use close_n1_stream() */
      com->command_byte_ptr=NULL;
      break;
    case GNU_ARJ_MODE_7:
      com->n_ptr=ARJ_NPT;
...
#ifndef NOT_USE_STD_compress_n0
				...
				if (kar > (NLIT-1))
				{
					...
					if(len>0)
					{
						...
						{ /* conversie naar 1 of 2? */
							if(kar_1==2)
							{ /* conversie naar 2? */
								if((n1_ptr_len(q[-1])+n1_len_len(3)+1)>18) /* TODO: groter dan lengte twee literals, moet afhankelijk gemaakt worden van de lengte van de pointer run */
								{ /* conversie naar 2 mogelijk */
									if(bp[-1]>0)
									{ /* extra conversie? */
										c_codetype kar_2=p[-2]+MIN_MATCH-NLIT-bp[-1];
