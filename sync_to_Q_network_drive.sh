...
rsync -a -v -W --copy-links -c --stats --progress '--exclude=.git/' '--exclude=*.[ao]' '--exclude=*.l[ao]' '--exclude=*.bak' '--exclude=*.i' '--exclude=*~' '--exclude=autom4te.cache/*' '--exclude=*.lock*' '--exclude=*~lock*'   . ~/q/nyh/Atari/e/pc/gup/
